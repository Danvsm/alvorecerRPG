-- Text-only Orkutista posts share the feed lifecycle without creating
-- placeholder or orphaned Storage objects.

alter table public.community_posts
  alter column image_path drop not null;

alter table public.community_posts
  add constraint community_posts_text_length_check
  check(image_path is not null or char_length(trim(caption))<=1000);

create or replace function alvorecer_private.queue_community_post_media()
