-- Persist each campaign member's cumulative Jokenpo winnings.
alter table public.campaign_members
  add column if not exists jokenpo_total_winnings_cents bigint not null default 0;

alter table public.campaign_members
  drop constraint if exists campaign_members_jokenpo_total_winnings_nonnegative;

alter table public.campaign_members
  add constraint campaign_members_jokenpo_total_winnings_nonnegative
  check (jokenpo_total_winnings_cents >= 0);

with totals as (
  select
    campaign_id,
    user_id,
    coalesce(sum(greatest(net_delta_cents, 0)), 0)::bigint as total_winnings_cents
  from public.jokenpo_rounds
  group by campaign_id, user_id
)
update public.campaign_members member
set jokenpo_total_winnings_cents = totals.total_winnings_cents
from totals
where member.campaign_id = totals.campaign_id
  and member.user_id = totals.user_id;
...
begin
  ...
  total_winnings_cents := actor_member.jokenpo_total_winnings_cents;
  ...
  if existing_round.id is not null then
    return jsonb_build_object(
      ...
      'max_bet_cents', least(
        200000,
        greatest(20000, (existing_round.balance_after / 1000) * 100)
      )
    );
  end if;
  ...
  if p_bet_cents is null or p_bet_cents < 20000 or p_bet_cents % 100 <> 0 then
    raise exception 'A aposta mínima é 200 Dracmas e deve ser um valor inteiro';
  end if;
  ...
  if source_before < 20000 then
    raise exception 'Você precisa de pelo menos 200 Dracmas para apostar';
  end if;

  max_bet_cents := least(200000, greatest(20000, (source_before / 1000) * 100));
  ...
  if daily_profit_cents + floor(p_bet_cents * 0.8)::bigint > 5000000 then
  ...
  if net_delta_cents > 0 then
    update public.campaign_members
    set jokenpo_total_winnings_cents =
      jokenpo_total_winnings_cents + net_delta_cents
    where campaign_id = p_campaign_id
      and user_id = actor_id
    returning jokenpo_total_winnings_cents into total_winnings_cents;
  end if;
  ...
  return jsonb_build_object(
    ...
    'balance_after', source_after,
    'total_winnings_cents', total_winnings_cents,
    'max_bet_cents', least(200000, greatest(20000, (source_after / 1000) * 100))
  );
end
$$;

revoke execute on function public.play_jokenpo(uuid, uuid, bigint, text, uuid)
  from public, anon;
grant execute on function public.play_jokenpo(uuid, uuid, bigint, text, uuid)
