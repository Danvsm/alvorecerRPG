-- Server-authoritative Jokenpo wagers using the existing Dracma wallets.

alter table public.dracma_transactions
  drop constraint if exists dracma_transactions_kind_check;
...
create table public.jokenpo_rounds (
  id uuid primary key default gen_random_uuid(),
  campaign_id uuid not null references public.campaigns(id) on delete cascade,
  user_id uuid not null references public.profiles(id) on delete cascade,
  character_id uuid references public.characters(id) on delete set null,
  request_id uuid not null,
  bet_cents bigint not null check (bet_cents between 100 and 5000),
  player_choice text not null check (player_choice in ('pedra', 'papel', 'tesoura')),
  tavern_choice text not null check (tavern_choice in ('pedra', 'papel', 'tesoura')),
  outcome text not null check (outcome in ('vitoria', 'derrota', 'empate')),
...
create index jokenpo_rounds_user_campaign_created_idx
  on public.jokenpo_rounds(user_id, campaign_id, created_at desc);

alter table public.jokenpo_rounds enable row level security;
revoke all on public.jokenpo_rounds from public, anon, authenticated;
grant select on public.jokenpo_rounds to authenticated;
grant all on public.jokenpo_rounds to service_role;

create policy jokenpo_rounds_read on public.jokenpo_rounds
for select to authenticated using (
  user_id = (select auth.uid()) or public.is_master(campaign_id)
);
...
begin
  ...
  select
    coalesce(sum(greatest(round.net_delta_cents, 0)), 0),
    coalesce(sum(greatest(-round.net_delta_cents, 0)), 0)
  into daily_profit_cents, daily_loss_cents
  from public.jokenpo_rounds round
  where round.user_id = actor_id
  ...
  if daily_loss_cents + p_bet_cents > 20000 then
    raise exception 'Seu limite de perdas das últimas 24 horas foi alcançado';
  end if;
